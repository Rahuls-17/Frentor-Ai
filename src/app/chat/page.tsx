...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    // Load saved study ref (book/chapter/verses) only — no study "type" anymore
    try {
  ...
  const onSend = async (text: string) => {
    ...
    try {
      ...
      // Fetch profile data from localStorage and parse safely
      const profileData = (() => {
        try {
      ...
      const res = await sendToBackend({
        ...
        mode: currentMode,
        answerMode,
        study: currentMode === "study" ? { ref: studyRef } : undefined, // no type
        profile: profileData,
      } as any);
          ...
          <div className={styles.studyBar}>
            <div className={styles.studyRow}>
              <div className={styles.studyField}>
                <label className={styles.studyLabel}>Book</label>
                <input
              ...
              <div className={styles.studyField}>
                <label className={styles.studyLabel}>Chapter</label>
                <input
              ...
              <div className={styles.studyField}>
                <label className={styles.studyLabel}>Verse(s)</label>
                <input
                  className={styles.studyInputSmall}
                  value={studyVerses ?? ""}
              ...
              {/* Removed Q&A / 3-min Sermon options */}
            </div>
